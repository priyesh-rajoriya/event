...
import {
  ...
  Dimensions,AsyncStorage, Alert,BackHandler
} from 'react-native';
import { Container, Header, Content, Footer, FooterTab, Icon, Button1, Badge, Card } from 'native-base';
...
export default class NonProfit extends Component {
  ...
  async componentWillMount() {
    ...
    Org_id = await AsyncStorage.getItem('organizer_id');
    console.log(Org_id + 'organizer_id');
    if (Org_id != undefined) {
 ...
 CancilDialog(){
  ...
  hart(item){
    if (item.message == "You have allready like event" || item.message == "like event" ){
    is_like='true'
      }
   else{'false'}
     }  

  async  getNonProfitEvents() {
    ...
    data.append('user_id',this.state.user_id);
      ...
      console.log(data)
      await fetch('http://ec2-52-207-124-182.compute-1.amazonaws.com:3333/api/v1/getNonProfitEvents?user_id='+user_id,config).then(res => res.json())
        .then(res => {
          console.log(res);
          this.setState({
            data:res.data
...
async likeAndUnlike(value, value1){
  console.log(user_id, value, value1)
  if(user_id == undefined ||  user_id == null ||  user_id == ""){
    alert('Please login first,  to like this event.')
  }else{
    var temp;
    if(value1 == "0"){
        temp= false
    ...
    console.log(AllAPI.like+"?"+"event_id="+value.id+"&user_id="+user_id+"&is_like="+temp)
  ...
  render() {
    return (
      <View style={styles.container}>
          ...
          <Image style={{ height: 160, width: '102%', marginLeft: -2 ,position:'absolute',top:0}}
            resizeMode='stretch'
            source={require('../Images/bg.png')}/>

            <View style={{ width: width, flexDirection: "row", alignItems: 'center', justifyContent: 'center',marginTop:-30}}>
      ...
      <FlatList
          ...
          <Card style={{backgroundColor:'#FAFAFA',marginTop:10,marginLeft:10,marginRight:10}}>
            <Image style={{ height: 120, width:'100%', }}
             source={{uri: item.image_url}} />
                   <Image
                  style={{ height: 80, borderRadius:5,width: 105,alignSelf:'center',marginTop:-55 }}
                  source={require('../Images/smalllogo.png')}
                  />
            <View style={{ flexDirection: 'row', marginLeft:20 }}>
            <View style={{alignItems:'center',}}>
              <Text style={{ marginTop:5, fontSize:20, color:'#000', }}>{item.members}</Text>
              <Text style={{ marginTop:1, fontSize:16, color:'#D35400', }}>Members</Text>
            </View>
              <View style={{ marginLeft: 5, marginRight:5, marginTop:-15,width:'80%', alignSelf:'center' }}>
                <Text style={{ fontSize: 17, }}>{item.title}</Text>
              </View>
              </View>
                <View style={{ flexDirection: 'row',marginTop:-25, marginLeft:70, alignSelf:"center"}}>
                  <View>
                    <Text style={{ fontSize: 16, color: '#CACFD2',marginTop: 5 }}>set, {item.end_date}{item.end_month},-{item.from_time}-{item.to_time}</Text>
                    <Text style={{ fontSize: 16, color: '#CACFD2', }}>{item.vanue}</Text> 
                  </View>
                {/*
            ...
            <View style={{marginTop:5,marginLeft:15,marginRight:15,backgroundColor:'#CCD1D1',height:1}}>
             </View>
              <TouchableOpacity onPress={() => this.openList(item)}
                style={{alignSelf:'center',margin:10}}>
                <Text style={{fontSize:15, alignSelf:'center',color:'#D35400'}}> Become A Member</Text>
              </TouchableOpacity>
          </Card>
            }
        ...
        <View style={{ width: width, justifyContent: 'space-around', backgroundColor: "#fff",
            ...
            <View style={{ alignItems: 'center' }}>
                <TouchableOpacity //onPress={() => this.props.navigation.navigate("NonProfitEventDetails")}
                    style={{ marginRight: 0, }}>
                    <Image resizeMode={'contain'}
                        source={require('../Images/handseak-icon-active.png')}
           ...
           <Dialog 
                ...
                /> 
        <Text style={{fontSize:15,alignSelf:'center',color:'#FF0000'}}>MEMBER INFO
        </Text>
                 <TextInput
                  style={{ fontSize: 15 , }}
                  placeholder="Email"
...
<View style={{flexDirection:'row',alignSelf:'center'}}>
                <Button onPress={() => this.CancilDialog()}
                style={{height:25,width:100,backgroundColor:'#FF0000'}}> 
                ...
                <Button style={{height:25,width:100, marginBottom:50}}> 
                  <Text style={{color:'#000',fontSize:12}}>Cancel
                  </Text>
                </Button> 
