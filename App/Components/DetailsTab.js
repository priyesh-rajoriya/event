...
import { Container, Header, Content, Footer, FooterTab, Icon, Card, Tab, Tabs, Badge,width } from 'native-base';
...
async likeAndUnlike(value, value1){
  ...
  console.log(AllAPI.like+"?"+"event_id="+value.id+"&user_id="+this.state.id+"&is_like="+temp)
  await fetch(AllAPI.like+"?"+"event_id="+value.id+"&user_id="+this.state.id+"&is_like="+temp)
    .then(res => res.json())
    .then(res => {
      console.log(res);
      this.getEvents();
    ...
    return (
      ...
      <ScrollView>
        <View style={{ flexDirection: 'row', marginTop: 15, alignItems:'center'}}>
          <Image style={{ height: 80, width: 80, marginLeft: 20, backgroundColor: 'transparent' }}
            source={require('../Images/Profile-image.png')} />
          <View style={{marginLeft:10,}}>
            <Text style={{ fontSize: 20, color: '#E59866', }}>Jeson Roy</Text>
            <Text style={{ fontSize: 12, color: '#E59866', marginTop: 5 }}>26 years</Text>
          </View>
        </View>

        <View style={{ flexDirection: "row", marginTop: 20 }}>
          <Text style={{ fontSize: 15, marginLeft: 20, color: '#E59866' }}> Total Hours  Volunteered</Text>
          <Text style={{ fontSize: 15, position: 'absolute', right: 20, color: '#E59866' }}> 10 Hours</Text>
        </View>
