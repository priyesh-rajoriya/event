...
var user_id, Org_id, event_id, is_like;
...
export default class Events extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      Eventid: '',
      event_category_id: '1',
      start_time: '2018-02-03 15:06:00',
      end_time: '2018-02-03 15:30:00',
      image_url: 'dadad',
      publish_status: '1',
      event_id: '',
      follower_id: '3',
  ...
  async  organizersList() {
    ...
    console.log(config)
    await fetch('http://ec2-52-207-124-182.compute-1.amazonaws.com:3333/api/v1/organizersList?user_id=' + user_id, config).then(res => res.json())
      .then(res => {
        AsyncStorage.setItem('organizer_id', res.data[0].organizer_id);
  ...
  hart(rowData) {
    if (rowData.message == "You have allready like event" || rowData.message == "like event") {
      is_like = 'true'
    }
    else { 'false' }
  }
  ...
  openList(item) {
    ...
    AsyncStorage.setItem('event_id', item.id.toString());
 ...
 async selectedListItem(item) {
    ...
    await fetch(AllAPI.like + "?" + "event_id=" + item.id + "&user_id=" + user_id + "&is_like=" + temp)
      .then(res => res.json())
      .then(res => {
        console.log(res);
        this.getEvents();
              ...
              <View
                ...
                <TouchableOpacity
                  ...
                  <Image
                    ...
                    source={require('../Images/Search01.png')} />
                </TouchableOpacity>
              </View>
            </Header>
            <View>
              <View
                ...
                {this.state.cityname == undefined &&
                  <TouchableOpacity
                    ...
                    <Text style={{ fontSize: 30, height: 40, maxWidth: 100, marginLeft: 5, color: '#fff' }}>India</Text>
                  </TouchableOpacity>
                }
                {this.state.cityname != undefined &&
                  <TouchableOpacity
                    style={styles.button}
                    onPress={() => this.openSearchModal()}>
                    <Text style={{ fontSize: 30, maxWidth: 100, marginLeft: 5, color: '#fff' }}>{this.state.cityname}</Text>
                  </TouchableOpacity>
                }
                <Image
                  style={{
                  ...
                  source={require('../Images/pancil.png')} />
              </View>
            </View>
            <View
              style={{
                            ...
                            <View style={{
                              ...
                              <Text
                                style={{
                                  ...
                                  color: '#DC7633',
                                  backgroundColor: 'transparent'
                                }}>{item.end_month.slice(0, 3)} {this.state.isLoading}</Text>
                              <Text style={{
                                fontSize: 18,
                                color: '#000'
                              }}>
            ...
            <FlatList
              ...
              renderItem={({ item }) =>
                ...
                <View
                  ...
                  <View style={{ justifyContent: 'center', marginTop: 2, alignItems: 'center' }}>
                    <TouchableOpacity onPress={() => this.follow(item, "0")}>
                      <View>
                        {item.is_follow == false && <Image
                          style={{
                            height: 30,
                            width: 110,
                          }}
                          source={require('../Images/followbtn_outerline.png')} />}
                      </View>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => this.follow(item, "1")}>
                      <View>
                        {item.is_follow == true && <Image
                          style={{
                            height: 30,
                            width: 110,
                          }}
                          source={require('../Images/followbtn.png')} />}
                      </View>
                    </TouchableOpacity>
                  </View>
                </View>
          ...
          <View
            ...
            <View style={{
              ...
              <TouchableOpacity onPress={() => this.Home()}>
                <Image
                  source={require('../Images/home-icon-red_03.png')}
                  style={{
                    width: 30,
                    height: 30
                  }} />
            ...
            <View style={{
              ...
              <TouchableOpacity
                onPress={() => this.props.navigation.navigate("NonProfit")}
                style={{
                  marginRight: 0
                }}>
                <Image
